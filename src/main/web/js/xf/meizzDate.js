...
Ver 2.1 
修改日期：2007-04-25
修改作者: 刘键 ztxr_liujian@163.com
修改内容：
1.更改了document.onclick和document.onkeyup的声明方式，使之符合语法。
2.添加了关闭层前的是否有层判断，修正错误。
3.修正了部分变量的声明，采用先声明后使用，使之符合语法。
...
strFrame = '<style>';
strFrame += 'INPUT.button{BORDER-RIGHT: #336699 1px solid;BORDER-TOP: #336699 1px solid;BORDER-LEFT: #336699 1px solid;';
strFrame += 'BORDER-BOTTOM: #336699 1px solid;BACKGROUND-COLOR: #fff8ec;font-family:宋体;}';
...
strFrame += '<table border=1 cellspacing=0 cellpadding=0 width=142 height=160 bordercolor=#336699 bgcolor=#336699 Author="wayx">';
...
strFrame += '      <tr align=center Author="wayx"><td width=16 align=center bgcolor=#336699 style="font-size:12px;cursor: pointer;color: #ffffff" ';
strFrame += '        onclick="parent.meizzPrevM()" title="向前翻 1 月" Author=meizz><b Author=meizz>&lt;</b>';
strFrame += '        </td><td width=60 align=center style="font-size:12px;cursor:default" Author=meizz ';
// this style.backgroundColor is for the button that is selected.
strFrame += 'onmouseover="style.backgroundColor=\'#336699\'" onmouseout="style.backgroundColor=\'white\'" ';
...
strFrame += '<td width=48 align=center style="font-size:12px;cursor:default" Author=meizz onmouseover="style.backgroundColor=\'#336699\'" ';
strFrame += ' onmouseout="style.backgroundColor=\'white\'" onclick="parent.tmpSelectMonthInnerHTML(this.innerText.length==3?this.innerText.substring(0,1):this.innerText.substring(0,2))"';
strFrame += '        title="点击这里选择月份"><span id=meizzMonthHead Author=meizz></span></td>';
strFrame += '        <td width=16 bgcolor=#336699 align=center style="font-size:12px;cursor: pointer;color: #ffffff" ';
...
strFrame += '<table border=1 cellspacing=0 cellpadding=0 bgcolor=#336699 ' + (bMoveable ? 'onmousedown="DragStart()" onmouseup="DragEnd()"' : '');
strFrame += ' BORDERCOLORLIGHT=#C1D2E4 BORDERCOLORDARK=#C1D2E4 width=140 height=20 Author="wayx" style="cursor:' + (bMoveable ? 'move' : 'default') + '">';
...
strFrame += '    <table border=1 cellspacing=2 cellpadding=0 BORDERCOLORLIGHT=#C1D2E4 BORDERCOLORDARK=#C1D2E4 bgcolor=#fff8ec width=140 height=120 Author="wayx">';
var n = 0;
for (j = 0; j < 5; j++) {
...
strFrame += '        <td colspan=3 align=right Author=meizz bgcolor="#336699"><span onclick=parent.closeLayer() style="font-size:12px;cursor: pointer;color:#FFFFFF;background-color:#336699"';
strFrame += '         Author=meizz title="' + _VersionInfo + '">关闭</span>&nbsp;</td></tr>';
strFrame += '    </table></td></tr><tr Author="wayx"><td Author="wayx">';
// This bgcolor is for the buttom line of button table.
strFrame += '        <table border=0 cellspacing=1 cellpadding=0 width=100% Author="wayx" bgcolor="C1D2E4">';
strFrame += '          <tr Author="wayx"><td Author=meizz align=left><input style="background-color:#336699; color:#FFFFFF" Author=meizz type=button class=button value="<<" title="向前翻 1 年" onclick="parent.meizzPrevY()" ';
strFrame += '             onfocus="this.blur()" style="font-size: 12px; height: 20px"><input style="background-color:#336699; color:#FFFFFF" Author=meizz class=button title="向前翻 1 月" type=button ';
strFrame += '             value="< " onclick="parent.meizzPrevM()" onfocus="this.blur()" style="font-size: 12px; height: 20px"></td><td ';
strFrame += '             Author=meizz align=center><input style="background-color:#336699; color:#FFFFFF" Author=meizz type=button class=button value=Today onclick="parent.meizzToday()" ';
strFrame += '             onfocus="this.blur()" title="当前日期" style="font-size: 12px; height: 20px; cursor:hand"></td><td ';
strFrame += '             Author=meizz align=right><input style="background-color:#336699; color:#FFFFFF" Author=meizz type=button class=button value=" >" onclick="parent.meizzNextM()" ';
strFrame += '             onfocus="this.blur()" title="向后翻 1 月" class=button style="font-size: 12px; height: 20px"><input ';
strFrame += '             style="background-color:#336699; color:#FFFFFF" Author=meizz type=button class=button value=">>" title="向后翻 1 年" onclick="parent.meizzNextY()"';
...
//lj rewrite function name in 2007-4-25 for correct the error
...
{
    //lj added if... in 2007-4-25 for correct the error 
    if (document.getElementById("meizzDateLayer") != null)
        document.all.meizzDateLayer.style.display = "none";
}

function IsPinYear(year) //判断是否闰平年
{
    //lj rewrite if-else in 2007-4-25 for simplify
...
{
    var dt;
    dt = new Date(year, month - 1, day).getDay() / 7;
    return dt;
}
...
function meizzToday() //Today Button
{
    ...
    if (outObject) {
        ...
        if (meizzTheMonth < 10) {
            tempMonth = tempMonth + meizzTheMonth;
        } else {
            tempMonth = meizzTheMonth;
        }
        //alert(today);
        if (today < 10) {
            tempToday = tempToday + today;
        } else {
            tempToday = today;
        }
        outObject.value = meizzTheYear + "" + tempMonth + "" + tempToday;
        if (typeof(outObject) != "undefined")
        {
            if (typeof(outObject.onchange) != "undefined" && outObject.onchange != null)
            {
                outObject.onchange();
            }
        }
...
function meizzSetDay(yy, mm) //主要的写程序**********
{
    ...
    for (i = 0; i < 39; i++)
    {
        ...
        {
            //初始化边框
            da.borderColorLight = "#C1D2E4";
            da.borderColorDark = "#C1D2E4";
            if (i < firstday)        //上个月的部分
            {
                ...
                if (!outDate)
                    da.style.backgroundColor = ((mm == 1 ? yy - 1 : yy) == new Date().getFullYear() &&
                                                (mm == 1 ? 12 : mm - 1) == new Date().getMonth() + 1 && meizzWDay[i] == new Date().getDate()) ? "#336699" : "#e0e0e0";
                else
                {
                    da.style.backgroundColor = ((mm == 1 ? yy - 1 : yy) == outDate.getFullYear() && (mm == 1 ? 12 : mm - 1) == outDate.getMonth() + 1 &&
                                                meizzWDay[i] == outDate.getDate()) ? "#9DBBD9" :
                                                                                     (((mm == 1 ? yy - 1 : yy) == new Date().getFullYear() && (mm == 1 ? 12 : mm - 1) == new Date().getMonth() + 1 &&
                                                                                       meizzWDay[i] == new Date().getDate()) ? "#336699" : "#e0e0e0");
                    //将选中的日期显示为凹下去
            ...
            else if (i >= firstday + GetMonthCount(yy, mm))        //下个月的部分
            {
                ...
                if (!outDate)
                    da.style.backgroundColor = ((mm == 12 ? yy + 1 : yy) == new Date().getFullYear() &&
                                                (mm == 12 ? 1 : mm + 1) == new Date().getMonth() + 1 && meizzWDay[i] == new Date().getDate()) ? "#336699" : "#e0e0e0";
                else
                {
            ...
            else        //本月的部分
            {
                ...
                if (!outDate)
                    da.style.backgroundColor = (yy == new Date().getFullYear() && mm == new Date().getMonth() + 1 && meizzWDay[i] == new Date().getDate()) ? "#336699" : "#e0e0e0";
                else
                {
                    da.style.backgroundColor = (yy == outDate.getFullYear() && mm == outDate.getMonth() + 1 && meizzWDay[i] == outDate.getDate()) ? "#9DBBD9" : ((yy == new Date().getFullYear() && mm == new Date().getMonth() + 1 && meizzWDay[i] == new Date().getDate()) ? "#336699" : "#e0e0e0");
                    //将选中的日期显示为凹下去
